function makeorder_babyloc_2_shortblock_dyna_runs(participant, user)
%% Generates 2 runs/CSV scripts for dynamic condition for the infant scans containing 1 stimuli per block with presentation rates of
%% .25 and a blank block between every other block.
%
...
for r = 1:nruns
    condvec = [randperm(4) randperm(4) randperm(4) randperm(4) randperm(4) randperm(4) randperm(4) randperm(4)];    % generate the order of the stim presentation
    % Check for consecutive repetitions and reshuffle if found
    while any(diff(condvec) == 0)
        condvec = [randperm(4) randperm(4) randperm(4) randperm(4) randperm(4) randperm(4) randperm(4) randperm(4)];
    end
...
for r = 1:nruns
    ...
    csvfilename = fullfile(participant_folder, strcat('script_babyloc_dyna_shortblock_run', num2str(run_number), '.csv'));
    ...
    for i = 1:ntrials 
        ...
        % Update onset time for the next baseline block
        onset_time = onset_time + blank_dur;

        % Update block num time for the next baseline block
        baseline_block = current_block + 1;

        %generate blank block 
        %Insert baseline block before the main loop
        random_index = randi([1, 48]);
        % Construct the image name with the current loop index
        blank_vid_name = strcat(blank_base_name, "-", num2str(random_index), ".mp4"); %generate image name for the current trial
