...
for run_num in range(run_num, 11):
    # loop thru the following steps if run is static and short
    if run_num % 2 == 0 and 5 <= run_num <= 8 :
        # ======================== experiment set-up ============================ 
        ...
        # ===== Generate par file for static short run ========================
         # Define color codes
        color_codes = {
            0: '0 0 0',     # blank
            1: '1 0 0',     # faces
        ...
        output_parfile_path = _thisDir + '/data/' + str(participant) + '/' + 'combined' + '/' + 'run' + str(run_num) + '.par'
    ...
    # loop thru the following steps if run is dynamic and short 
    elif run_num % 2 != 0 and 5 <= run_num <= 8 : 
                # ======================== experiment set-up ============================ 
        # Dynamically generate the file path based on run number
        ...
        countdown_images = [visual.ImageStim(win, str(path), flipVert=True, size=(768,768)) for path in countdown_path.glob('*.png')]
         ...
         # ======================== Generate dynamic short parfiles ============================
        # Create an empty list to store parfile data
        par_file_data = []
        ...
        #loop thru if run_num is static and long
        # ======================== experiment set-up ============================ 
        # Dynamically generate the file path based on run number
        ...
        # ===== Generate par files for static long run =====
        color_codes = {
            0: '0 0 0',     # blank
            1: '1 0 0',     # faces
            2: '0.8 0.8 0', # hands
    ...
    # ====== Generate parfiles for dynamic long runs  =============
        # Create an empty list to store parfile data
        par_file_data = []
        ...
        # ======================== experiment set-up ============================ 
        # Dynamically generate the file path based on run number
        csv_file_path = _thisDir + '/data/' + str(participant) + '/' + 'grayscale' + '/' + 'script_babyloc_dyna_long_run' + str(run_num) + '.csv'
        ...
        # Concatenate strings
        experiment_info_header = participant_str + '_run' + str(run_num) + '_' + expName + '_' + date_str
        filename = _thisDir + '/data/' + str(participant) + '/' + 'grayscale' + '/' + str(experiment_info_header)
        ...
        par_file_df.to_csv(_thisDir + os.sep + u'data/' + str(participant) + '/' + 'grayscale' '/' + experiment_info_header + 'dynamic_par_file.par', sep='\t', index=False, header=False, lineterminator='\n')
