% patterened last image is added to last blank trial instead of being independent
...
for r = 1:nruns
    condvec = [shuffle([1:5]) shuffle([1:5]) shuffle([1:5])  shuffle([1:5]) shuffle([1:5]) shuffle([1:5]) shuffle([1:5])  shuffle([1:5])]    % generate the order of the stim presentation
    % Check for consecutive repetitions and reshuffle if found
    while any(diff(condvec) == 0)
        condvec = [shuffle([1:5]) shuffle([1:5]) shuffle([1:5]) shuffle([1:5]) shuffle([1:5]) shuffle([1:5]) shuffle([1:5])  shuffle([1:5])];
    end
...
for r = 1:nruns
    ...
    for i = 1:ntrials
         if condmat(i, r) == 5 % blank category
            img_name = imgmat{i, r};
            if i == ntrials && condmat(i, r) == 5
                % For the last trial in the last blank block, select a random image
                random_image_number = randi(7); % Generate a random number between 1 and 7
                img_name = strcat('last_image_', num2str(random_image_number), '.jpg');
            end
