...
function makeorder_babyloc_static_short(participant, usr)
...
cats = {'blank', 'faces', 'hands', 'cars', 'scenes'}; % Updated category order
...
npadblocks = 1; % number of extra baseline blocks at the beginning and end
TR = 2; % fMRI TR (secs)
propodd = 0.5;
...
for r = 1:nruns
    condvec = [shuffle(1:ncats), shuffle(1:ncats), shuffle(1:ncats), shuffle(1:ncats)]; % generate the order of the stim presentation
    % Check for consecutive repetitions and reshuffle if found
    while any(diff(condvec) == 0)
        condvec = [shuffle(1:ncats), shuffle(1:ncats), shuffle(1:ncats), shuffle(1:ncats)];
    end
...
for r = 1:nruns
    ...
    for tri = 1:ntrials
        if condmat(tri, r) == 1 % for the blank category (category number 0)
            counter = counter + 1;
...
for r = 1:nruns
    ...
    if r <= 2
        csvfilename = fullfile(participant_folder, strcat('script_babyloc_static_short_run', num2str(r), '.csv'));
    ...
    elseif 3 <= r && r <= 4
        ...
        fprintf(fid, 'Block #,Onset-time(s),Category,TaskMatch,Image Name,Image Path\n');
    elseif 5 <= r && r <= 6
        csvfilename = fullfile(participant_folder, strcat('script_babyloc_static_short_run', num2str(r + 4), '.csv'));
        fid = fopen(csvfilename, 'w');
        fprintf(fid, 'Block #,Onset-time(s),Category,TaskMatch,Image Name,Image Path\n');
    elseif 7 <= r
        csvfilename = fullfile(participant_folder, strcat('script_babyloc_static_short_run', num2str(r + 6), '.csv'));
        fid = fopen(csvfilename, 'w');
        fprintf(fid, 'Block #,Onset-time(s),Category,TaskMatch,Image Name,Image Path\n');
    end
    for i = 1:ntrials
