...
%
% INPUT: Should be the baby's number and the usr of your laptop
...
cats = {'blank', 'faces', 'hands', 'cars', 'scenes'};
...
participant_folder = fullfile('/Users', usr, 'Desktop', 'alternating_bb', 'alternating_PsychoPy', 'data', participant, 'mid');
...
padblocks = repmat(1,npadblocks,1); % vector of baseline blocks to pad run with
for r = 1:nruns
    condvec = [shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats])  shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats])  shuffle([1:ncats])]    % generate the order of the stim presentation
    % Check for consecutive repetitions and reshuffle if found
    while any(diff(condvec) == 0)
        condvec = [shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats])  shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats])  shuffle([1:ncats])];
    end
...
for r = 1:nruns
    ...
    for tri = 1:ntrials
        if condmat(tri, r) == 1 % for the blank category
            counter = counter + 1;
            % Ensure that stimnums(counter) does not exceed the maximum
            imgmat{tri, r} = strcat(cats{1}, '-', num2str(mod(start_stim + counter - 1, max_stim_per_category) + 1), '.jpg');
        end
    end
    ...
    for cat = 2:ncats
        ...
        for tri = 1:ntrials
            if condmat(tri, r) == cat %update category numbers
                counter = counter + 1;
                % Ensure that stimnums(counter) does not exceed the maximum
...
image_directory = fullfile('/Users', usr, 'Desktop', 'alternating_bb', 'alternating_stimuli',  'static_stimuli')
...
for r = 1:nruns
    ...
    if r <= 2
        csvfilename = fullfile(participant_folder, strcat('script_babyloc_static_mid_run', num2str(r), '.csv'));
    ...
    elseif 3 <= r && r <= 4
        csvfilename = fullfile(participant_folder, strcat('script_babyloc_static_mid_run', num2str(r+2), '.csv'));
        fid = fopen(csvfilename, 'w');
        fprintf(fid, 'Block #,Onset-time(s),Category,TaskMatch,Image Name,Image Path\n');
    %Make it so ninth and tenth runs in participant's folder are static
    elseif 5 <= r && r <= 6
        csvfilename = fullfile(participant_folder, strcat('script_babyloc_static_mid_run', num2str(r+4), '.csv'));
        fid = fopen(csvfilename, 'w');
        fprintf(fid, 'Block #,Onset-time(s),Category,TaskMatch,Image Name,Image Path\n');
    %Make it so 13 and 14th runs in participant's folder are static
    elseif 6 <= r
        csvfilename = fullfile(participant_folder, strcat('script_babyloc_static_mid_run', num2str(r+6), '.csv'));
        fid = fopen(csvfilename, 'w');
        fprintf(fid, 'Block #,Onset-time(s),Category,TaskMatch,Image Name,Image Path\n');
    end
    
    for i = 1:ntrials
    if condmat(i, r) == 1 % blank category
        img_name = imgmat{i, r};
    ...
    fprintf(fid, '%i,%f,%i,%s,%s,%s\n', ...
        ...
        condmat(i, r)-1,... % write trial condition, subtract 1 so conditions are reindexed 0 to 4
        cats{condmat(i, r)},... % write stimulus category
        img_name, ... % write video file name
        img_path); % write full video path
    end
    
    % Add the last image to the same CSV file for each run
    last_image_number = randi(7); % Generate a random number between 1 and 7
    last_image_name = strcat('last_image_', num2str(last_image_number), '.jpg');
    last_image_path = fullfile(image_directory, last_image_name);

    % Calculate the duration of the last image
    last_image_duration = stimdur;

    fprintf(fid, '%i,%f,6,last,%s,%s\n', nblocks, rundur, last_image_name, last_image_path); % Append the last image entry
