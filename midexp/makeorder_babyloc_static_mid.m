function makeorder_babyloc_static_mid(participant, user)
...
participant_folder = fullfile('/Users', user, 'Desktop', 'alternating_bb', 'alternating_PsychoPy', 'data', participant, 'mid');
...
for r = 1:nruns
    condvec = [randperm(ncats), randperm(ncats), randperm(ncats), randperm(ncats), randperm(ncats), randperm(ncats), randperm(ncats), randperm(ncats)]    % generate the order of the stim presentation
    % Check for consecutive repetitions and reshuffle if found
    while any(diff(condvec) == 0)
        condvec = [randperm(ncats), randperm(ncats), randperm(ncats), randperm(ncats), randperm(ncats), randperm(ncats), randperm(ncats), randperm(ncats)];
    end
...
image_directory = fullfile('/Users', user, 'Desktop', 'alternating_bb', 'alternating_stimuli',  'static_stimuli')
