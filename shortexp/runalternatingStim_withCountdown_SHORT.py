...
expName = 'shortbabyloc'  # from the Builder filename that created this script
expInfo = {
...
# --- Setup the Window ---
win = visual.Window(
    size=(1920, 1080), checkTiming=False, fullscr=0, screen=1, #change screen stuff here; size=(960, 540), screen=0 is default--- trying to get the stimuli to play only on the scanner screen
...
for run_num in range(1, 17):
    # loop thru the following steps if run_num is static <=2 
    if run_num <= 2 or (5 <= run_num <= 6) or (9 <= run_num <= 10) or (13 <= run_num <= 14):
        # ======================== experiment set-up ============================ 
        # Dynamically generate the file path based on run number
        csv_file_path = _thisDir + '/data/' + str(participant) + '/' + 'short' + '/' + 'script_babyloc_static_2Hz_run' + str(run_num) + '.csv'
        ...
        filename = _thisDir + '/data/' + str(participant) + '/' + 'short' + '/'+ str(experiment_info_header)
...
#        # --- Setup the Window ---
#        win = visual.Window(
#            size=(1920, 1080), checkTiming=False, fullscr=True, screen=1, #change screen stuff here; size=(960, 540), screen=0 is default--- trying to get the stimuli to play only on the scanner screen
#            winType='pyglet', allowStencil=False, 
#            monitor='testMonitor', color=[0, 0, 0], colorSpace='rgb',
#            backgroundImage='/Users/vpnl/Desktop/static_bb/static_stimuli/controlled_backgrounds/background_color.png', backgroundFit='contain', 
#            blendMode='avg', useFBO=True, 
#            units='pix')
#        win.mouseVisible = True
#        # store frame rate of monitor if we can measure it
...
#        
        ...
        countdown_images = [visual.ImageStim(win, str(path), flipVert=False) for path in countdown_path.glob('*.png')]
        ...
        par_file_df.to_csv(_thisDir + os.sep + u'data/' + str(participant) + '/' + 'short' + '/' + experiment_info_header + 'static_par_file.par', sep='\t', index=False, header=False, lineterminator='\n')
    ...
    # loop thru the following steps if run_num >=2
                ...
                # ======================== experiment set-up ============================ 
        # Dynamically generate the file path based on run number
        csv_file_path = _thisDir + '/data/' + str(participant) + '/' + 'short' + '/' + 'script_babyloc_dyna_2Hz_run' + str(run_num) + '.csv'
    
        columns_to_import = ["Block #", "Category", "Onset-time(s)", "Video Name", "Video Path", "TaskMatch"]
        ...
        thisExp = data.ExperimentHandler(name=expName, version='',
            extraInfo=expInfo, runtimeInfo=None,
            originPath=f'/Users/{usr}/Desktop/dynamic_bb',
            savePickle=True, saveWideText=True,
            dataFileName=filename)
...
#        defaultKeyboard = keyboard.Keyboard(backend='iohub')
        ...
        # export conditions par file
        par_file_df.to_csv(_thisDir + os.sep + u'data/' + str(participant) + '/' + 'short' '/' + experiment_info_header + 'dynamic_par_file.par', sep='\t', index=False, header=False, lineterminator='\n')
        print("Par file exported successfully!")
