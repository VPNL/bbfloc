...
[Iteration 2](https://github.com/VPNL/bbfloc/blob/main/README.md#using-iteration-1)) Taking into account the need for longer runs with a more recurring visual baseline condition, we generated two new experiment versions. These two versions have the same run length (4 minutes and 30 seconds) and only vary in their block duration (4s vs 8s) and present a baseline block after every stimulus block.
