...
participant_folder = fullfile('/Users', usr, 'Desktop', 'alternating_bb', 'alternating_PsychoPy', 'data', participant, 'long');
...
for r = 1:nruns
    condvec = [shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats])  shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats])  shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats])  shuffle([1:ncats])]    % generate the order of the stim presentation
    % Check for consecutive repetitions and reshuffle if found
    while any(diff(condvec) == 0)
        condvec = [shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats])  shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats]) shuffle([1:ncats])  shuffle([1:ncats])];
    end
...
for r = 1:nruns
    ...
    if r <= 2
        csvfilename = fullfile(participant_folder, strcat('script_babyloc_static_long_run', num2str(r), '.csv'));
    ...
    elseif 3 <= r && r <= 4
        csvfilename = fullfile(participant_folder, strcat('script_babyloc_static_long_run', num2str(r+2), '.csv'));
        fid = fopen(csvfilename, 'w');
        fprintf(fid, 'Block #,Onset-time(s),Category,TaskMatch,Image Name,Image Path\n');
    %Make it so ninth and tenth runs in participant's folder are static
    elseif 5 <= r && r <= 6
        csvfilename = fullfile(participant_folder, strcat('script_babyloc_static_long_run', num2str(r+4), '.csv'));
        fid = fopen(csvfilename, 'w');
        fprintf(fid, 'Block #,Onset-time(s),Category,TaskMatch,Image Name,Image Path\n');
    ...
    last_image_name = strcat('last_image_', num2str(last_image_number), '.png');
