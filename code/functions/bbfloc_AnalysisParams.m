function [session, init_params, glm_params] = bbfloc_AnalysisParams(session, clip, setup)
% 
...
%
% [session, init_params, glm_params] = bbfLocAnalysisParams(session, clip)
% 
% INPUTS
% 1) session: fullpath to scanning session directory in ~/fLoc/data/ that 
%             is being analyzed (string)
% 2) clip: # of TRs to clip from beginnning for each each run
% 
% 
% OUPUTS
% 1) session: fullpath to scanning session directory in ~/fLoc/data/ that 
...
init_params = bbfloc_initDefaultParams;
...
% Extracting only the first four characters of the first element in each array
if numel(prts1) >= 1
    prts1{1} = prts1{1}(1:min(4, length(prts1{1})));
end

if numel(prts2) >= 1
    prts2{1} = prts2{1}(1:min(4, length(prts2{1})));
end

block_dur = round(str2double(prts2{1}) - str2double(prts1{1}), 1); epb = block_dur / TR;
epb = round(block_dur / TR);
if rem(epb, 1) > 0
    fprintf('Error: TR must be a whole number'); return;
end
...
% clipping
init_params.clip        = clip;     % array of the number of TRs to clip from beginning of each run (initialize as 0)
...
% motion compensation
data = niftiRead(niipaths{1});
n_vols = size(data.data, 4);
init_params.motionCompRefFrame     = 6; % frame number of reference TR for within-scan compensation
...
% else % set init_params.keeFrames using the value of the "clip" field set above
%     init_params.keepFrames = zeros(num_runs, 2);
%     for i = 1:num_runs
%         init_params.keepFrames(i, :) = [clip(i,:)];
%     end
% end 
else
    % Pad clip rows to the same length (e.g., length 6)
    %clip = [clip, NaN(num_runs, 6 - size(clip, 2))];  % Adjust length if needed
    
    init_params.keepFrames = zeros(num_runs, 2);
    
    for i = 1:num_runs
        init_params.keepFrames(i, :) = clip(i, 1:2);  % Use first 2 elements
    end
end
...
init_params.description = 'bbfloc'; % char array describing session
init_params.comments    = 'Analyzed using with beths new dynacat code edited for bbfloc'; % char array of comments
init_params.annotations = annotations; % cell array of descriptions for each run
init_params.vAnatomy = fullfile(setup.vistaDir, setup.subID, setup.sessionID, '3DAnatomy/t1.nii.gz');
...
% if dataTYPES(1).scanParams.nFrames < 20
glm_params.detrend       = 1;  % detrending procedure: -1 = linear, 0 = do nothing, 1 = high-pass filter, 2 = quadratic
glm_params.detrendFrames = 8;
% else
%     glm_params.detrend       = 1;  % detrending procedure: -1 = linear, 0 = do nothing, 1 = high-pass filter, 2 = quadratic
%     glm_params.detrendFrames = 20; %20; % cutoff for high-pass filter in cycles/run (if glm_params.detrend = 1)
% end
...
glm_params.glmHRF        = 5;  % set the HRF: 0 = deconvolve, 1 = estimate HRF from mean response, 2 = Boynton 1996, 3 = SPM, 4 = Dale 1999; 5 = infant; 6 = insub HRF default = 3 
...
% GLM annotation
glm_params.annotation = sprintf('bbflocGLM_%iruns',length(init_params.functionals));
